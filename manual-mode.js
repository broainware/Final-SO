...
function addProcess() {
    ...
    const dependencies = [];
    document.querySelectorAll('#dependencies-list input[type="checkbox"]:checked').forEach(checkbox => {
...
function checkDeadlock() {
    ...
    if (result.hasDeadlock) {
        const nodeNames = result.cycleNodes
            .map(id => processes.find(p => p.id === id)?.name || id)
            .join(' → ');
...
function updateDependenciesList() {
    ...
    container.innerHTML = processes.map(process => `
        <div class="checkbox-item">
            <input type="checkbox" id="dep-${process.id}" value="${process.id}">
            <label for="dep-${process.id}">${process.name}</label>
        </div>
    `).join('');
}
